...
class DataService {
    ...
    static getApiUrl() {
        ...
        // Vercel يستخدم /api/gas، وNetlify يستخدم /_api/gas.
        // الـ Proxy يضيف GAS_URL وGAS_TOKEN من Environment Variables على السيرفر.
        const h = window.location.hostname;
        if (h.includes('netlify.app') || h.endsWith('.netlify.app')) return '/_api/gas';
        return '/api/gas';
    }
    ...
    static async _loadFromGAS() {
        ...
        // أي تعديلات يديرها الأدمن على المجموعات/الغرف تُحفظ في SiteConfig
        // وتُطبق فوق الملف المحلي. لو السحابة غير متاحة نستخدم الملف المحلي كـ fallback.
        try {
            const remoteConfig = await this._fetchSiteConfig();
            if (remoteConfig && remoteConfig.groups) structData.groups = remoteConfig.groups;
            if (remoteConfig && remoteConfig.rooms) structData.rooms = remoteConfig.rooms;
            if (remoteConfig && remoteConfig.meta) structData.meta = { ...structData.meta, ...remoteConfig.meta };
            structData._siteConfig = remoteConfig || {};
        } catch (e) {
            console.warn('DataService: SiteConfig غير متاح، استخدام البيانات المحلية:', e.message);
            structData._siteConfig = {};
        }

        // Google Sheets هو المصدر الوحيد للمشاركين. لا نستخدم نسخة محلية
        ...
        try {
            ...
            structData._gasError = err?.message || 'تعذر الاتصال بـ Google Sheets';
            throw new Error('فشل تحميل بيانات Google Sheets: ' + structData._gasError);
        }
    ...
    static async _fetchGASParticipants(url) {
        ...
        const request = async (method, endpoint, options = {}) => {
            ...
            try {
                ...
                if (!res.ok) throw new Error('HTTP ' + res.status);
                const json = await res.json();
                if (json && json.status === 'success' && Array.isArray(json.data)) return json.data;
                throw new Error((json && json.message) || 'استجابة غير صالحة من Google Apps Script');
            } finally {
                clearTimeout(tid);
            }
        };

        // القراءة المباشرة من Google Apps Script تستخدم GET أولاً؛ هذا أكثر ثباتاً
        // على GitHub Pages لأنه لا يحتاج POST/CORS preflight.
        if (isDirect) {
            const sep = targetUrl.includes('?') ? '&' : '?';
            try {
                return await request('GET', targetUrl + sep + 'action=getAll', {
                    headers: { 'Accept': 'application/json' }
                });
            } catch (getErr) {
                // fallback قديم: POST text/plain
                return await request('POST', targetUrl, {
    ...
    static async sendToGAS(payload) {
        ...
        const sensitiveIndividual =
            ...
            /^(attendance\d+|pamphlet)$/.test(String(cleanPayload.category || ''));
        if (!publicActions.has(action) || sensitiveIndividual) {
            cleanPayload.sessionToken = sessionStorage.getItem('admin_session_token') || '';
        }
    ...
    static _buildLocalParticipants(confData) {
        const result = [];
        const seen = new Set();
        const add = (name, extra = {}) => {
            ...
            if (seen.has(key)) {
                const existing = result.find(p => p.name.toLowerCase() === key);
                if (existing) Object.assign(existing, extra);
                return;
            }
            seen.add(key);
            result.push({
                id: 'local-' + result.length + '-' + key.replace(/[^a-z0-9\u0600-\u06ff]+/gi, '-'),
                name: clean,
                group: extra.group || '',
        ...
        // المصدر الأول: تقسيمة المجموعات الموجودة في conference-data.json
        (Array.isArray(confData.groups) ? confData.groups : []).forEach(g => {
            (Array.isArray(g.members) ? g.members : []).forEach(name => add(name, {
                group: g.name,
                groupId: g.id
            }));
        });

        // المصدر الثاني: التسكين الموجود في نفس الملف.
        // يضيف أي أسماء موجودة في الغرف وليست مذكورة داخل تقسيمة المجموعات.
        (Array.isArray(confData.rooms) ? confData.rooms : []).forEach(room => {
            (Array.isArray(room.persons) ? room.persons : []).forEach(name => {
                const clean = String(name || '').trim();
                if (!clean) return;
                const key = clean.toLowerCase();
                const existing = result.find(p => p.name.toLowerCase() === key);
                if (existing) {
                    ...
                    if (!existing.gender) existing.gender = room.gender || '';
                } else {
                    add(clean, {
                        room: room.name || '',
                        roomId: room.id || null,
    ...
    static _mergeGASIntoConference(confData, gasItems) {
        ...
        // المجموعات الموجودة في conference-data.json هي المرجع للأسماء والـ IDs.
        // لا نستخدم أسماء قديمة ثابتة هنا لأنها كانت سبباً في ربط أعضاء الشيت
        // بمجموعات مختلفة عن المعروضة للمستخدم.
        ...
        if (Array.isArray(confData.groups)) {
            confData.groups.forEach((g, index) => {
                if (!g || !g.id) return;
                const id = String(g.id).trim();
                const name = String(g.name || id).trim();
                groupIdToName.set(id, name);
                groupNameToId.set(normalizeGroupKey(id), id);
                groupNameToId.set(normalizeGroupKey(name), id);
                // دعم الصيغة الشائعة في Google Sheets: مجموعة 1 / مجموعة 2...
                groupNameToId.set(normalizeGroupKey(`مجموعة ${index + 1}`), id);
                groupNameToId.set(normalizeGroupKey(`group ${index + 1}`), id);
                // أسماء قديمة كانت مستخدمة في بعض نسخ Google Sheets؛ تُقبل كـ aliases
                // بدون تغيير الاسم الظاهر للمجموعة في الموقع.
                const legacyAliases = [
                    ['مارجرجس', 'القلب'],
                    ['مريم العذراء', 'الفكر'],
                    ['الشهيده مارينا', 'الشهيدة مارينا', 'الارادة', 'الإرادة'],
                    ['البابا كيرلس', 'الراحة', 'الطريق']
                ][index] || [];
                legacyAliases.forEach(alias => groupNameToId.set(normalizeGroupKey(alias), id));
            });
        }
        ...
        if (Array.isArray(confData.groups)) {
            confData.groups.forEach(g => {
                (Array.isArray(g.members) ? g.members : []).forEach(member => {
                    memberToGroup.set(String(member).trim().toLowerCase(), {name:String(g.name||'').trim(), id:String(g.id||'').trim()});
                });
            });
        }
        ...
        gasItems.forEach(item => {
            ...
            const memberGroup = memberToGroup.get(key);
            const rawGroup = (memberGroup?.name || item.group || base.group || base.groupId || '').trim();
            const rawGroupKey = normalizeGroupKey(rawGroup);
            const matchedId = rawGroup
                ? (groupNameToId.get(rawGroupKey) || (groupIdToName.has(rawGroup) ? rawGroup : null))
                : null;
            const matchedName = matchedId ? (groupIdToName.get(matchedId) || rawGroup) : rawGroup;
    ...
    static _normalizeParticipants(dataCopy) {
        ...
        (Array.isArray(dataCopy.groups) ? dataCopy.groups : []).forEach((g, index) => {
            if (!g || !g.id) return;
            const id = String(g.id).trim();
            const name = String(g.name || id).trim();
            groupIdToName.set(id, name);
        ...
        dataCopy.participants.forEach(p => {
            ...
            if (rawGroup) {
                const matchedId = groupNameToId.get(normalizeGroupKey(rawGroup)) ||
                    (groupIdToName.has(rawGroup) ? rawGroup : null);
                p.groupId = matchedId || null;
                p.group = matchedId ? (groupIdToName.get(matchedId) || rawGroup) : rawGroup;
            } else {
                p.groupId = null;
                p.group = '';
            }
            ...
            if (!p.seatNumber && p.seat) {
                const seat = parseInt(p.seat);
                if (!isNaN(seat)) p.seatNumber = seat;
            }
